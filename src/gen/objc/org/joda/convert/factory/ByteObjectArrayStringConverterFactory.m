...
- (OrgJodaConvertFactoryByteObjectArrayStringConverterFactory_ByteArrayStringConverter_Enum)toNSEnum {
  return (OrgJodaConvertFactoryByteObjectArrayStringConverterFactory_ByteArrayStringConverter_Enum)[self ordinal];
}
...
- (IOSObjectArray *)convertFromStringWithIOSClass:(IOSClass *)cls
  ...
  if (JreIntMod([str java_length], 2) == 1) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"Invalid Byte[] string");
  }
  IOSObjectArray *array = [IOSObjectArray arrayWithLength:JreIntDiv([str java_length], 2) type:JavaLangByte_class_()];
  for (jint i = 0; i < array->size_; i++) {
